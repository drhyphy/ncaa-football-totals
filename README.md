...
The [separate season collection profile](model/reports/WEATHER_REVISION_SEASON_COLLECTION_PROTOCOL.md) continues September 16, 2026 at 03:00 UTC through February 1, 2027 at 03:00 UTC. The [input inventory](model/reports/WEATHER_REVISION_MODEL_INVENTORY.md) currently has no usable successive-run model inputs: both retained captures were manual and requested the same initialization. The proposed experimental fit starts only after 60 eligible games across two completed weeks, then uses fixed weekly refits and a February 8 evaluation. This is a practical fitting minimum, not evidence of an edge. The original pilot and the four current paper policies remain separately recorded.
