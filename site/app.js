...
(function (root, factory) {
  ...
  function start(doc) {
    ...
    function render(data) {
      ...
      renderPicks(new Date()); renderCandidates(); renderCalibrationStudy(); renderOrdinaryStudy(); renderDirectProbabilityStudy(); renderPbpStudy(); renderArchivedReplay(); renderPerformance(); renderTransparency();
    }
    ...
    fetch(`data/research.json?refresh=${Date.now()}`, {cache: "no-store"}).then(response => { if (!response.ok) throw new Error(`HTTP ${response.status}`); return response.json(); }).then(data => {research = data; if (board) {renderWeather(new Date()); renderCalibrationStudy(); renderOrdinaryStudy(); renderDirectProbabilityStudy(); renderPbpStudy(); renderArchivedReplay();}}).catch(() => {});
