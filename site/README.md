...
The forward table deduplicates on `(model_version, candidate, game_id)` using the earliest `recorded_at`. The runtime remains responsible for storing immutable original selection terms and updating their grades. Aggregate headline results are for the primary candidate named in `performance.candidate`, defaulting to `opponent_adjusted_ridge`.

The active v4 model uses two distinct books and separates quote-change timestamps from recent full-state receipts. Earlier historical metrics are quarantined when `historical_evidence_status` reports market contamination. `market_opportunities` holds the separate exact two-leg scanner; dominance and negative hedge floors are not presented as profitable bets. The separate capture workflow preserves near-kickoff observations without refitting models.
