...
## Follow-up schema check — September 9, 2026 UTC

Before the separately planned authenticated preflight, the current vendor reference and OpenAPI were rechecked. This follow-up made no authenticated requests.

| Endpoint | Exact query fields and response contract |
|---|---|
| `/v3/historical/events` | Required strings: `apiKey`, `sport`, `league`, `from`, `to`; RFC3339 date range, maximum 31 days. Returns an array of `dto.SimpleEventDto`: numeric `id`, names/IDs, `date`, sport/league, status and scores. No pagination parameter appears on this endpoint. [Reference](https://docs.odds-api.io/api-reference/historical/get-historical-events). |
| `/v3/historical/odds` | Required strings: `apiKey`, `eventId`, `bookmakers` (comma-separated, maximum 30); optional `markets` string, so `markets=Totals` is documented. Returns one `controllers.EventResponse`, including bookmaker market arrays. [Reference](https://docs.odds-api.io/api-reference/historical/get-historical-odds). |
| `/v3/odds/movements` | Required strings: `apiKey`, `eventId`, `bookmaker`, `market`; `marketLine` is a string without OpenAPI `required:true`, although its description exempts only ML. Returns `controllers.HandicapMovementsResponse`, with `opening`, `latest`, and `movements[]`. No time-range or state filter. [Reference](https://docs.odds-api.io/api-reference/odds/get-odds-movements). |

**The historical-odds contract does directly expose total-side prices.** Its schema resolves through `EventResponse.bookmakers[book][]` → `dto.MultiMarketDto.odds[]` → `dto.Odds`, containing `over` and `under` strings and numeric `hdp`. This differs from the generic movement-row schema. `updatedAt` belongs to the market object and is only typed as a string. There is no documented main-line flag, array-order guarantee, or required set of side fields. Therefore inspect actual rows; do not substitute `home`/`away`. [OpenAPI schemas](https://docs.odds-api.io/api-reference/openapi.json).

Filter the exact full-event market name `Totals`, not period or team-total variants. The guide illustrates a totals line as `max`, while the current generic schema also contains `hdp` and an integer `max`; this documentation inconsistency requires retaining the raw row and checking its field meaning. If several lines are returned, a frozen deterministic choice can identify a **retention-probe line**, without claiming it was the main line or using it as an earlier decision-price sample. [Market names and response example](https://docs.odds-api.io/guides/fetching-odds).

No new movement timestamp, revision, suspension or withdrawal guarantee was found. The OpenAPI explicitly defines Unix milliseconds for the *different* dropping-odds fields `lastMovedAt`/`updatedAt`, but leaves movement `timestamp` as an unexplained integer. Those units cannot be transferred across endpoints. A successful history response would still establish retrieved provider records, not continuous price availability. [OpenAPI timestamp schemas](https://docs.odds-api.io/api-reference/openapi.json).
